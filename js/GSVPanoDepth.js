...
GSVPANO.PanoDepthLoader = function (parameters) {
    ...
    this.load = function(panoId) {
        if(panoId in ids){
            if(this.onDepthLoad){
                console.log("skipping loaded depthMap: " + panoId);
                this.onDepthLoad(false);
                return;
            }
        }
        ids[panoId] = 1;
        ...
        $.ajax({
            ...
            .done(function(data, textStatus, xhr) {
                ...
                try {
                    ...
                    if(self.onDepthLoad){
                        self.onDepthLoad(false);
                    }
                    return;
                    //data = self.createEmptyDepthMap();
                }
                if(self.onDepthLoad) {
                    self.depthMap = data.depthMap;
                    self.planes = data.planes;
                    self.indices = data.indices;
                    self.onDepthLoad(true);
                }
            })
    ...
    this.computeDepthMap = function(header, indices, planes) {
        ...
        for(y=0; y<h; ++y) {
            for(x=0; x<w; ++x) {
                ...
                if(planeIdx > 0) {
                    ...
                    if(!(planeIdx in planeDepth)){
                        planeDepth[planeIdx] = {
                            total: 0,
                            count: 0,
                        }
                    }
                    planeDepth[planeIdx].total += Math.sqrt(v[0]*v[0] + v[1]*v[1] + v[2]*v[2]);
                    planeDepth[planeIdx].count += 1;
                } else {
        ...
        for(y=0; y<h; ++y) {
            for(x=0; x<w; ++x) {
                var planeIdx = indices[y*w + x];
                var plane = planes[planeIdx];
                var avgDepth = planeDepth[planeIdx].total / planeDepth[planeIdx].count;
                if(avgDepth > 30 || Math.abs(plane.n[2]) > 0.5){
                    depthMap[y*w + (w-x-1)] = 0;
                }
                else{
                    depthMap[y*w + (w-x-1)] = planeIdx;
                }
            }
        }
    ...
    this.parse = function(depthMap) {
        ...
        return {depthMap: depthMap, planes: data.planes, indices: data.indices};
    }
