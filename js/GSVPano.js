...
GSVPANO.PanoLoader = function (parameters) {
    ...
    this.load = function (location) {
        ...
        var self = this;
        _panoClient.getPanoramaByLocation(location, 100, function (result, status) {
            if (status === google.maps.StreetViewStatus.OK) {
                ...
                self.lat = result.location.latLng.k;
                self.lng = result.location.latLng.A;
                self.pitch = result.tiles.originPitch;
                self.heading = result.tiles.originHeading;
                self.composePanorama();
            } else {
                if( self.onNoPanoramaData ) self.onNoPanoramaData( status );
                self.throwError('Could not retrieve panorama for the following reason: ' + status);
                if (self.onPanoramaLoad) {
                    self.onPanoramaLoad(false);
                }
            }
        });
